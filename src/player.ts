...
export class Player extends Body {
    ...
    public update() {
        if (this.keyboarder.isDown(KEYS.LEFT) && this.center.x - this.size.x / 2 > 0) {
            this.center.x -= 2;
        }
        else if (this.keyboarder.isDown(KEYS.RIGHT) && this.center.x + this.size.x / 2 < this.game.gameSize.x) {
            this.center.x += 2;
        }
